...
namespace API.Controllers
{
    ...
    public class ClientBooksController : ControllerBase
    {
        ...
        public async Task<ActionResult<ClientBook>> BorrowBook([FromBody] ClientBook clientBook)
        {
            ...
            // Validate that client doesn't already have an unreturned copy of this book
        ...
        /// <summary>
        /// Get all borrowings for a specific client (REPLACES the removed endpoint from ClientsController)
        /// </summary>
        [HttpGet("client/{clientId}")]
        [ProducesResponseType(typeof(IEnumerable<ClientBook>), StatusCodes.Status200OK)]
        public async Task<ActionResult<IEnumerable<ClientBook>>> GetByClient(int clientId)
        {
            _logger.LogInformation("Getting all borrowings for client ID: {ClientId}", clientId);

            var query = new GetListGenericQuery<ClientBook>(
                condition: cb => cb.ClientId == clientId,
                includes: q => q
                    .Include(cb => cb.Client)
                    .Include(cb => cb.Book),
                orderBy: q => q.OrderByDescending(cb => cb.BorrowedDate)
            );
        ...
        /// <summary>
        /// Get currently borrowed books for a specific client (active only)
        /// </summary>
        [HttpGet("client/{clientId}/active")]
        [ProducesResponseType(typeof(IEnumerable<ClientBook>), StatusCodes.Status200OK)]
        public async Task<ActionResult<IEnumerable<ClientBook>>> GetActiveByClient(int clientId)
        {
            _logger.LogInformation("Getting active borrowings for client ID: {ClientId}", clientId);

            var query = new GetListGenericQuery<ClientBook>(
                condition: cb => cb.ClientId == clientId && !cb.IsReturned,
                includes: q => q
                    .Include(cb => cb.Client)
                    .Include(cb => cb.Book),
                orderBy: q => q.OrderBy(cb => cb.DueDate)
        ...
        /// <summary>
        /// Get borrowing history for a specific client (returned books only)
        /// </summary>
        [HttpGet("client/{clientId}/history")]
        [ProducesResponseType(typeof(IEnumerable<ClientBook>), StatusCodes.Status200OK)]
        public async Task<ActionResult<IEnumerable<ClientBook>>> GetHistoryByClient(int clientId)
        {
            _logger.LogInformation("Getting borrowing history for client ID: {ClientId}", clientId);
            ...
            var query = new GetListGenericQuery<ClientBook>(
                ...
                includes: q => q
                    .Include(cb => cb.Client)
                    .Include(cb => cb.Book),
                orderBy: q => q.OrderByDescending(cb => cb.ReturnedDate)
            );
        ...
        /// <summary>
        /// Get all borrowings for a specific book
        /// </summary>
        [HttpGet("book/{bookId}")]
        [ProducesResponseType(typeof(IEnumerable<ClientBook>), StatusCodes.Status200OK)]
        public async Task<ActionResult<IEnumerable<ClientBook>>> GetByBook(int bookId)
        {
            _logger.LogInformation("Getting all borrowings for book ID: {BookId}", bookId);

            var query = new GetListGenericQuery<ClientBook>(
                condition: cb => cb.BookId == bookId,
                includes: q => q
                    .Include(cb => cb.Client)
                    .Include(cb => cb.Book),
                orderBy: q => q.OrderByDescending(cb => cb.BorrowedDate)
        ...
        /// <summary>
        /// Get currently borrowed instances of a specific book (active only)
        /// </summary>
        [HttpGet("book/{bookId}/active")]
        [ProducesResponseType(typeof(IEnumerable<ClientBook>), StatusCodes.Status200OK)]
        public async Task<ActionResult<IEnumerable<ClientBook>>> GetActiveByBook(int bookId)
        {
            _logger.LogInformation("Getting active borrowings for book ID: {BookId}", bookId);

            var query = new GetListGenericQuery<ClientBook>(
                condition: cb => cb.BookId == bookId && !cb.IsReturned,
                includes: q => q
                    .Include(cb => cb.Client)
                    .Include(cb => cb.Book),
                orderBy: q => q.OrderBy(cb => cb.DueDate)
